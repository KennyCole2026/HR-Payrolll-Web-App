...
async function main() {
  ...
  if (!connected) { console.error('   Supabase connection failed. Continuing with report generation...\n'); }
  ...
  writeFileSync(join(outputsDir, 'sample.html'), html);
  console.log('   HTML generated: ./outputs/sample.html');
  console.log('3. Generating Excel report...');
  const excelBuffer = await generateApprovalScheduleExcel(sampleInput);
  writeFileSync(join(outputsDir, 'sample.xlsx'), Buffer.from(excelBuffer));
  console.log('   Excel generated: ./outputs/sample.xlsx');
